...
- Paths use **host repo root** (`paths.repo_root_host` in `platform.runtime.json`), not n8n container `/data/scripts`. Run `amctl apply` after cloning or moving the repo.
- **automedia** MCP reads `data/runs` on the host; keep `./data/runs` shared with n8n/gateway (compose mounts) so Plan B and MCP see the same files.
- **n8n-mcp** is for Cursor only (`.mcp.json`); Hermes does not use it. See [N8N_MCP.md](N8N_MCP.md) and [MCP_AUTOMEDIA.md](MCP_AUTOMEDIA.md).
- Verify: `bash scripts/verify_mcp_evidence.sh` after apply.
