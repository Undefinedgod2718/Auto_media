#!/usr/bin/env bash
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/lib/common.sh"
source "$(dirname "${BASH_SOURCE[0]}")/lib/read-platform.sh"

RUN_ID=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run-id) RUN_ID="$2"; shift 2 ;;
    *) shift ;;
  esac
done

[[ -n "$RUN_ID" ]] || json_err "missing --run-id"

RUN_DIR="$(ensure_run_dir "$RUN_ID")"
SVG="${RUN_DIR}/art.svg"
PNG="${RUN_DIR}/post.png"

if [[ ! -f "$SVG" ]]; then
  # PNG-first flows may already provide post.png directly; art.svg is optional.
  [[ -f "$PNG" ]] && { json_ok "$PNG"; exit 0; }
  json_err "missing image source: need post.png or art.svg"
fi

if [[ "${AUTO_MEDIA_MOCK:-0}" == "1" ]]; then
  cp "$SVG" "${RUN_DIR}/post.mock.svg"
  python3 - <<PY || json_err "mock png failed"
from pathlib import Path
# minimal valid 1x1 PNG
png = bytes([
  0x89,0x50,0x4E,0x47,0x0D,0x0A,0x1A,0x0A,0x00,0x00,0x00,0x0D,0x49,0x48,0x44,0x52,
  0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x01,0x08,0x02,0x00,0x00,0x00,0x90,0x77,0x53,
  0xDE,0x00,0x00,0x00,0x0C,0x49,0x44,0x41,0x54,0x08,0xD7,0x63,0xF8,0xCF,0xC0,0x00,
  0x00,0x03,0x01,0x01,0x00,0x18,0xDD,0x8D,0xB4,0x00,0x00,0x00,0x00,0x49,0x45,0x4E,0x44,
  0xAE,0x42,0x60,0x82,
])
Path("${PNG}").write_bytes(png)
PY
  [[ -f "$PNG" ]] || json_err "png not created"
  json_ok "$PNG"
  exit 0
fi

BINARY="$(engine_binary render)"
require_cmd "$BINARY"

"$BINARY" -w 1080 -h 1080 "$SVG" -o "$PNG" || json_err "rsvg-convert failed"
[[ -f "$PNG" ]] || json_err "png not created"

json_ok "$PNG"
